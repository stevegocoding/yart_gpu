...
__global__ void kernel_gen_primary_rays(uint res_x, uint res_y,
										...
										c_ray_chunk out_chunk)
{
	...
	if (pixel_idx < res_x * res_y)
	{
		...
		uint x = 0, y = 0;
		uint scr_pos = 0; // Starting with lsb bit 0.
		uint target_pos = 0;
		uint mask = 1;
		...
		float3 pt_raster_world = dfunc_transform_pt(con_mat_cam2world.elems, pt_raster); 
