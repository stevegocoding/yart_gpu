...
//
//	/*
//	cp.compact(d_identity.buf_ptr()(), d_is_valid, old_count, d_out_src_addr, d_new_count.buf_ptr());
//
...
//	cuda_safe_call_no_sync(cudaMemcpy(d_stencil.buf_ptr(), stencil_array, num_elems*sizeof(uint32), cudaMemcpyHostToDevice));
//	c_cuda_memory<uint32> d_out_compacted(num_elems);
//	c_cuda_memory<uint32> d_new_count(1); 
//	cuda_safe_call_no_sync(cudaMemset(d_out_compacted.buf_ptr(),0,num_elems*sizeof(uint32)));
//	// device_compact<uint32>(d_array, d_stencil.buf_ptr()(), num_elems, d_out_compacted.buf_ptr(), d_new_count.buf_ptr());
//	uint32 new_count = cuda_gen_compact_addresses(d_stencil.buf_ptr()(), num_elems, d_out_compacted.buf_ptr());
//	cuda_compact_in_place<uint32>(d_array, d_out_compacted.buf_ptr()(), num_elems, new_count);
